...
class PropertyWidget(QWidget):
    def __init__(
        ...
        if self._prop_type is PropertyType.Float:
            ...
            self._value_widget.setValue = self._value_widget.setCurrentText
            self._value_widget.value = self._value_widget.currentText
        else:
            self._value_widget = QLineEdit()
            self._value_widget.setValue = self._value_widget.setText
            self._value_widget.value = self._value_widget.text
    ...
    def _on_prop_changed(self, device, prop, value):
        if device == self._device_label and prop == self._prop_name:
